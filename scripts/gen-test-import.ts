...
function buildUniform(ws: ExcelJS.Worksheet, dataRows: Row[]) {
  ...
  ws.addRow(["A12345", "男", "L", 1, "新領", 32, 30, 1, "新領", "（範例列）"]);
  for (const r of dataRows) ws.addRow(r);
  ws.getRow(1).fill = HEADER_FILL;
  ws.getRow(1).font = { bold: true };
  ws.getRow(2).fill = NOTE_FILL;
...
const VALID_UNIFORM: Row[] = [
  // 上衣 + 折褲，qty 下界
  ["RT020466", "男", "S", 1, "新領", 28, 28, 1, "新領", "下界尺寸組合"],
  // 上衣 + 折褲，qty 上界
  ["RT020205", "男", "3XL", 5, "新領", 42, 36, 5, "新領", "上界尺寸組合"],
  // 只申請上衣
  ["RT018378", "男", "XL", 1, "新領", "", "", "", "", "賴允瀚 - 只要上衣"],
  // 只申請折褲
  ["RT020374", "男", "", "", "", 34, 32, 1, "新領", "只要折褲"],
  // 自購（不觸發 warn）
  ["RT019574", "男", "M", 2, "自購", 32, 30, 2, "自購", "自購情境"],
];
...
const ERR_UNIFORM: Row[] = [
  ...
  // r10 折褲腰圍錯誤
  ["RT020205", "男", "", "", "", 27, 30, 1, "新領", "腰圍不在清單"],
  // r11 折褲褲長錯誤
  ["RT020205", "男", "", "", "", 32, 40, 1, "新領", "褲長不在清單"],
  // r12 折褲 qty 錯誤
  ["RT020205", "男", "", "", "", 32, 30, 10, "新領", "件數超過 5"],
  // r13 折褲 action 錯誤
  ["RT020205", "男", "", "", "", 32, 30, 1, "送修", "折褲領用方式錯誤"],
  // r14 上衣 action = 更換 → warn
  ["RT020466", "男", "L", 1, "更換", "", "", "", "", "⚠ warn：含更換"],
  // r15 valid（上衣+折褲）
  ["RT020466", "男", "L", 2, "新領", 32, 30, 1, "新領", "✅ valid"],
  // r16 valid（只上衣）
  ["RT018378", "男", "XL", 1, "新領", "", "", "", "", "✅ valid 只上衣"],
  // r17 valid（只折褲）
  ["RT020374", "男", "", "", "", 34, 32, 1, "新領", "✅ valid 只折褲"],
];
